...
async function initCompanionChatPage() {
  ...
  function renderCompanionLivePanel(data) {
    ...
    const roleLabel =
      data.role === 'COMPANION' ? 'Companion' : data.role === 'CUSTOMER' ? 'Khách' : escapeHtml(data.role || '');
  ...
  async function resubscribeCompanionLiveSocket() {
    ...
    try {
      await RealtimeStomp.connect();
      liveStompSub = await RealtimeStomp.subscribeBookingLocation(currentBookingId, (payload) =>
        renderCompanionLivePanel(payload)
      );
    } catch (e) {
